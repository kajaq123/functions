...
app.geometry(f"{window_width}x{window_height}+{(screen_width-window_width)//2}+{(screen_height-window_height)//2}")
...
def start():
    ...
    for i in range(0, window_width, scale):
        canvas.create_line(
            i,
            center_y - line_l,
            i ,
    ...
    for i in range(0, window_height, scale):
        canvas.create_line(
            center_x - line_l,
            i ,
            center_x + line_l,
...
def function2(y):
    ...
    for x in arange(width*-1, width,scale): # clever system for idk what :)
        function_list.extend([x+ width, negative(((x/scale/4) **y))*scale+ height])  # Add each point as a separate coordinate
